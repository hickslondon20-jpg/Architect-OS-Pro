...
ones). Reference: @/tmp/gsd-install/package/.claude/gsd-core/references/edge-probe.md.
...
# — NOT the consuming project's git root — falling back to git toplevel / /tmp/gsd-install/package/.claude.
...
EDGE_PROBE_JS=$(for _c in \
  ...
  "/tmp/gsd-install/package/.claude/gsd-core/bin/lib/edge-probe.cjs" \
  "/tmp/gsd-install/package/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)
...
requirement list). Reference: @/tmp/gsd-install/package/.claude/gsd-core/references/prohibition-probe.md — the
...
Use the SPEC.md template from @/tmp/gsd-install/package/.claude/gsd-core/templates/spec.md.
