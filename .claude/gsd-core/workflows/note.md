...
- **Global scope**: `/tmp/gsd-install/package/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/tmp/gsd-install/package/.claude/notes/`)
...
2. Glob `/tmp/gsd-install/package/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/tmp/gsd-install/package/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/tmp/gsd-install/package/.claude/notes/` — works in any directory
