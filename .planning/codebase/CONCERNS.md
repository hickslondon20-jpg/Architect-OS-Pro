...
- **Both `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` are invalid/expired on the live Railway production deployment (confirmed 2026-07-08, MA-03 Objective 4 live trigger test; RE-CONFIRMED STILL INVALID at 21:24 UTC and again at 21:33 UTC after a code push/redeploy in between).** Deploy logs for `api.architectospro.com` show a real request (`POST /api/wiki/compile-event`) failing with `openai.AuthenticationError: Error code: 401 - Incorrect API key provided` during embedding generation (masked suffix `...RXcA`), immediately preceded by an Anthropic `401 invalid x-api-key` during Claude synthesis for the same request. **Founder confirmed (2026-07-08) that the Railway Variables tab has the correct variable names (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY` - verified against `core/config.py`'s `validation_alias`, no naming mismatch) and that the key VALUES match what's in `python-backend/.env` and `.env.local`.** A diagnostic grep of `python-backend/.env` to compare the masked suffix confirmed the local OpenAI key's suffix matches the `...RXcA` seen in every Railway 401 - the correct value is deployed. That grep also printed both full plaintext key values into the agent transcript (should have checked variable names only, same class of mistake as the earlier Vault secret exposure - logged as its own line item below). **Refined diagnosis: since the correct key value is confirmed deployed and still rejected by the provider's own API with "Incorrect API key provided," the most likely explanation is the keys were revoked or regenerated at the provider level (OpenAI Platform / Anthropic Console) after being captured into these `.env` files, not a Railway configuration problem.** This isn't a code bug (the code handles both failures exactly as designed: the Anthropic failure triggers the intended mechanical-fallback path, and the OpenAI failure now falls back to a null embedding without crashing - see Fixed This Session above), but it means **no real AI synthesis or embedding call has yet succeeded in production** - a launch blocker independent of any specific feature. **Before beta launch:** log into platform.openai.com/account/api-keys and console.anthropic.com directly and confirm whether these specific keys still show as active; if not, generate fresh ones there, update `python-backend/.env`, `.env.local`, and Railway's Variables tab together, trigger a redeploy, and re-verify with a real live call (not just app startup). When updating Railway, paste the new value directly into Railway's UI - do not paste raw key values into an agent chat for comparison.
...
- Rotate all API keys/secrets before go-live: `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` on Railway are currently invalid in production (see Secrets and Credential Risk above), plus `ARCHITECTOS_INGEST_SECRET` and the Vault-stored `wiki_autotrigger_ingest_secret`.
