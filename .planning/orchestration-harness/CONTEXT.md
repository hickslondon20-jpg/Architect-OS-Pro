...
| # | Conflict | Resolution |
|---|---|---|
| O1 | Two VCSO implementations exist: live Python agentic loop (`/api/vcso/chat`, surface `virtual_cso`) vs. legacy Vercel single-shot (`api/vcso/chat.ts`, surface `ws5-chat`, 0 recent usage). **CLAUDE.md Rule #1 still calls the VCSO a Vercel serverless exception.** | **Phase 0 cleanup.** Live = Python. Retire/quarantine the dead Vercel `chat.ts`; correct CLAUDE.md Rule #1 to state the VCSO is Python-served. Do not build harness logic in the Vercel file. |
| O2 | Two wiki representations: `ose_knowledge_pages` (read by the VCSO prompt) vs. the `wiki_*` claim/evidence/digest system (via `per_user_wiki`). Authoritative source for query-time composition is ambiguous. | **Phase 0 cleanup.** Decide the authoritative query-time wiki + how the composer reads it, before leaning the cost model on wiki components. |
| O3 | Conversation→synthesis→OS-Engine→wiki feeder (compounding Layer-2 from chat) not confirmed running; wiki pages track uploads, not threads. | **Phase 0 verify.** Confirm (or scope) the conversation feeder; it is the build-time cost lever. Separate from the upload→wiki feeder which appears live. |
