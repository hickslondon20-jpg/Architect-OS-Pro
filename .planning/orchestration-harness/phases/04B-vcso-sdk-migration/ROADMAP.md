...
**Status (2026-07-22): delegation reliable (Gate 1 closed); delivery gate open (Gate 2).** SDK-M3 met
the reliability bar — **5/5 consecutive passes on a pinned anchor** (15/15 delegations allowed first-try,
zero denials; 15/15 workers completed; correct tiers). The load-bearing evidence is *not* the pass rate:
the **delegation order varied run-to-run (structured→wiki→sandbox ×3, structured→sandbox→wiki ×2) while
the sandbox-waits-for-structured dependency held every time** — the lead genuinely reasons the
decomposition, which a hardcoded sequence or Path A cannot, and which D2 existed to restore, evidenced for
the first time. **Defect 7 is closed in code** (per-`(turn,capability)` worker tokens; the existing scope
check now refuses cross-worker calls; unit-proven — the guard has not been *seen* firing since no run
attempted a cross-worker call). Effort-scaling holds both ways (app-gated → system-level restraint, not
proof the model would self-restrain — a Phase-G question). **Per Process Rule 10, Gate 1 (delegation
reliability) is CLOSED** and D2 graduates from "proven once" on delegation. **Gate 2 (founder-visible
delivery) is OPEN at 4/5:** run 4's SSE stream died mid-turn and the founder saw an error for a turn that
had actually succeeded and persisted (33 citations); **Defect 8** (fixed v0.6.100) addresses the
*consequence* (a lost paid-for answer), not the still-undetermined *cause*. **The flag's dark-exit now
gates on delivery closing** — observed Defect-8 recovery (via an *injected* disconnect, not chasing the
intermittent cause) plus a keepalive log-confirm from the existing runs — **alongside Defect 7.** Nothing
downstream (M4/E/F/G, any non-dark exposure) proceeds until delivery closes. Plans: `04B-D2-M3-PLAN.md`;
evidence: `04B-D2-M3-COMPLETION.md`.
