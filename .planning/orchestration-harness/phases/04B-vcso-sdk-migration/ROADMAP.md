...
| D2. Model-driven delegation (restore reasoning-driven selection) | **SDK-M2 CLOSED (2026-07-20) — Canary 5 PASS on deployed `56c8d604`/v0.6.76: first founder-visible cited answer on the model-driven path (parent run `28d1b9ce`, child `3f5554f0`; 5,242-char / 33-citation answer; lead reasoned, delegated via `Task`, worker fired from the subagent, child row completed). Flag re-darkened; `vcso_planner` retired. Defect 6 resolved & verified live. Open tier-2 follow-up: quantify the worker's actual contribution (child ran ~0.4s / no separate Haiku cost) before the three-worker chain. See `04B-D2-M2-FINISH-LOG.md` → "Canary 5". SDK-M3 not started.** | 2026-07-20 |
