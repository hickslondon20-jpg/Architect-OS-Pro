...
- Frontend URL: pending founder confirmation.
- Backend URL: `https://api.architectospro.com`.

## Supabase Auth

- Authentication > URL Configuration > Site URL: set to the live frontend URL.
- Authentication > URL Configuration > Redirect URLs: allow the live frontend URL and the future reset route:
  - `<live-frontend-url>`
  - `<live-frontend-url>/reset-password`
  - `<live-frontend-url>/#/reset-password` if the deployed app remains hash-routed for auth flows.
...
- `ARCHITECTOS_CORS_ORIGINS`: include the live frontend URL. Do not rely on localhost defaults.
...
- Report the frontend URL and the tested flow back to the agent for final Supabase/LangSmith outcome checks.
