...
**Surfaces.**
- **Domain Agents (primary home).** Skill authoring (skill-creator flow), the global vs
  private split (platform library vs founder-created), building-block files (a skill's
  bundled templates/scripts/brand rules), and the sandbox as the artifact renderer.
...
become registry citizens (resolves the Ep4 seam). **Sub-agents** advance indirectly
(registry discovery + bridge). **Artifacts** get supercharged — the bridge lets a single
sandbox execution assemble an artifact from live platform data.
...
1. Build the harness engine **generic**, not contract-review-specific — one engine for
   Domain Agent artifact workflows and (directionally) OS Engine synthesis.
2. Reconcile the **per-thread workspace with the persistent artifact library** (thread work
...
|---|---|---|---|
| D1 | M8 capability registry vs Ep5 tool registry — one layer or two | Two distinct layers | Ep5 build-planning |
